...
class WarpActivity extends Activity {
  ...
  override def onCreate(savedInstanceState: Bundle) {
    ...
    convert.setText(if(txt != null) txt else """
		    This program is covered under GPL v. 3 it can be downloaded [here](http://github.com/georgerogers42/warppipe)
		    """)
    ...
    val send = new Button(this)
    send.setText("Send")
    send.setOnClickListener((x : View) => {
      val action = new Intent()
      action.setAction(Intent.ACTION_SEND)
      action.putExtra(Intent.EXTRA_TEXT, result.getText)
